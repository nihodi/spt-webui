...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    environment.etc."spt-webui/env" =
      let base = "https://${cfg.settings.domain}${cfg.settings.baseHref}";
      in
     {
      text = ''
        FRONTEND_URL=${base}
  ...
  services.nginx =
    let
      ...
      frontend-env = spt-packages.frontend-env { apiPrefix = "https://${cfg.settings.domain}${cfg.settings.baseHref}/api"; };
